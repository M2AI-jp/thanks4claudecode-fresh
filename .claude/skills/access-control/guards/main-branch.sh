...
if [ "$CURRENT_BRANCH" != "main" ] && [ "$CURRENT_BRANCH" != "master" ]; then
    ALLOW=true # success return removed: consolidated allow exit below
fi
...
EOF
    ...
    if [ "$ALLOW" = false ] && { [ "$TOOL_NAME" = "Edit" ] || [ "$TOOL_NAME" = "Write" ]; }; then
    ...
    if [ "$ALLOW" = false ] && [ "$TOOL_NAME" = "Bash" ]; then
...
if [[ "$ALLOW" == true ]]; then
    # success return consolidated: multiple allow paths return here.
    exit 0
fi
