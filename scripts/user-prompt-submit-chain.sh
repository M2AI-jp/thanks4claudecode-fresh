#!/bin/bash
# user-prompt-submit-chain.sh - event unit: user-prompt-submit
# Symlinked from .claude/events/user-prompt-submit/chain.sh

set +e

# Resolve actual script location (follow symlink)
REAL_DIR="$(cd "$(dirname "$(readlink -f "${BASH_SOURCE[0]}")")" 2>/dev/null && pwd)" || REAL_DIR="."
REPO_ROOT="$(cd "$REAL_DIR/.." 2>/dev/null && pwd)" || REPO_ROOT="."
EVENT_DIR="$REPO_ROOT/.claude/events/user-prompt-submit"
