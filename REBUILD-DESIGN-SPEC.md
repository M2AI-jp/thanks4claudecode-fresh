...
>
> 参照先: BUILD-FROM-SCRATCH.md（構築手順） / EXAMPLE-CHATGPT-CLONE.md（運用脚本）
>
...
2. Module（単機能スクリプト）
3. SubAgent（独立した判断者）
4. Skill（Module + SubAgent のパッケージ）
5. Hook（自動化レイヤー）

これが「本来実現したかったこと」の概念レベルの整理だと思う。この整理が先にあって、初めて「何を Skill にするか」「何を SubAgent にするか」が決まる。
...
- reviewer は `docs/design/temporal-achievability-spec.md` を参照
...
この失敗は `play/archive/codex-usage-improvement/plan.json` に記録されている。**設計したルールが実装経路に乗っていない**ことが原因だった。
...
この失敗は `play/archive/m1-post-maintenance/plan.json` に記録されている。ここで初めて **state と git の整合**が仕様に組み込まれた。
