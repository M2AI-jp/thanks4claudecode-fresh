...
>
> **MECE 役割**: 概念定義・仕様の SSOT（概念マップ、Claude Code仕様、Evidence、Issue、playbook v2、時間的達成可能性）
>
> 読み順: README.md を参照
>
...
> - §2 背景と失敗のストーリー → **PROJECT-STORY.md が SSOT**（本文書は仕様視点の要約）
> - §3 Claude Code の仕様 → **本文書が SSOT**
> - §5 エンジニアリング概念マップ → **本文書が SSOT**
> - §7 失敗の構造（仕様） → **本文書が SSOT**（物語は PROJECT-STORY.md）
> - §8.2 コンポーネント仕様 → **BUILD-FROM-SCRATCH.md §6 が SSOT**（本文書は呼び出しルールのみ）
> - §9 Evidence と検証 → **本文書が SSOT**
> - §10-11 Temporal/Deadlock → **本文書が SSOT**
> - §14 playbook v2 雛形 → **本文書が SSOT**
...
> **SSOT 注記**: 物語の詳細は **PROJECT-STORY.md** を参照。本セクションは仕様視点の要約。
...
**（AIオーケストレーション観点の注釈：各フェーズは"人間タスク"に落とす）**

各フェーズはそのままモジュールにせず、以下のように「最小作業単位」に分解してから境界を決める。

| フェーズ | 最小作業単位への分解 |
| --- | --- |
| 要件定義 | 要件の抽出 → 曖昧点の質問 → 受入条件の作成 → スコープ境界の明記 |
| 設計 | 制約整理 → 代替案列挙 → 意思決定記録（ADR） → インタフェース定義 |
| 実装 | タスク分割 → 変更計画 → 差分実装 → セルフレビュー → テスト追加 |
| テスト | テスト設計 → 生成/実装 → 実行 → 失敗解析 → 修正 → 再実行 |
| デプロイ | リリース条件確認 → 変更手順実行 → ロールバック準備 → 監視確認 |
| 運用・保守 | アラート分類 → 原因仮説 → 暫定対応 → 恒久対応 → 振り返り |
...
**（AIオーケストレーション観点：領域は"依頼先エージェント/責務"の分割軸）**

領域分割は「担当者」ではなく「作業の型（入力→処理→出力→検証）」で分ける。

| 領域 | 作業の型への分解 |
| --- | --- |
| FE | UI 仕様化 → a11y チェック → 状態管理設計 → E2E 観点の受入条件化 |
| BE | API 契約 → 例外設計 → 認証認可 → データ整合性 → 観測（ログ/メトリクス） |
| DB | スキーマ差分 → マイグレーション計画 → インデックス/性能 → データ移行 |
| Infra | 環境差分 → スケール/耐障害 → 秘密情報 → ネットワーク/権限 |
| DevOps | CI ゲート設計 → 品質メトリクス → デプロイ手順 → 監視とアラート |
...
**（AIオーケストレーション観点：品質=「合否判定できるチェック」へ落とす）**

それぞれの品質特性は「自動判定できるチェック」または「人間の承認タスク」に変換する。

| 特性 | 自動判定できるチェックへの変換 |
| --- | --- |
| 機能性 | 受入条件のテストケース化、E2E のシナリオ網羅表 |
| 信頼性 | SLO/エラー予算、リトライ/タイムアウト設計のチェックリスト |
| 効率性 | 主要経路の性能予算（p95 など）と測定コマンド |
| 保守性 | lint/型/複雑度/依存の上限、変更容易性の指標 |
| セキュリティ | 脅威モデル簡易版、依存監査、権限最小化レビュー |
| 互換性 | 契約テスト（schema）、後方互換ポリシー |
| 可用性 | 監視項目、アラート閾値、ランブックの有無 |
...
**（AIオーケストレーション観点：テストは"生成→実行→判定→修正"に分割）**

テストを 1 モジュールにせず、以下の最小単位に分けると自動化しやすい。

| # | 最小単位 | 説明 |
| --- | --- | --- |
| 1 | テスト観点抽出 | 仕様/受入条件から観点を抽出 |
| 2 | テストケース設計 | 入力/期待/境界/例外を設計 |
| 3 | テスト実装 | コード化・データ準備 |
| 4 | 実行 | CI/ローカル/ステージング |
| 5 | 失敗の分類 | 環境/テスト不備/実装不備/仕様不備 |
| 6 | 修正提案 | 実装 or テスト or 仕様の修正 |
| 7 | 再実行・レポート化 | 差分と結論 |
...
**（AIオーケストレーション観点：レビューも"判定可能なチェック"へ）**

レビューは「指摘を生む」作業なので、チェックリスト化してエージェントで前処理できる。

| 対象 | チェックリスト例 |
| --- | --- |
| コード | lint/型/循環依存/複雑度/セキュリティ臭/命名/コメント/差分の説明有無 |
| 設計 | 責務境界、依存方向、非機能要件、失敗時挙動、観測性（ログ/メトリクス/トレース） |
| 要件 | 受入条件、例外系、用語定義、優先度、スコープ外の明記 |
| テスト | 境界値、回帰観点、失敗時のデバッグ容易性 |
| ドキュメント | 最新性、手順の再現性、図と用語の一致 |
...
**（AIオーケストレーション観点：タスクは"実行可能な粒度"まで落とす）**

「タスク」をさらに分解して、エージェントに委譲できる最小単位に揃える。

| # | 最小単位 | 説明 |
| --- | --- | --- |
| 1 | 情報収集 | 関連仕様/既存実装/依存の列挙 |
| 2 | 方針提案 | 複数案とトレードオフ |
| 3 | 実装計画 | 変更ファイル候補、影響範囲、ロールバック |
| 4 | 実装 | 差分作成 |
| 5 | 自己検証 | ビルド、lint、型、テスト |
| 6 | 変更説明 | PR 本文、リリースノート |
| 7 | レビュー対応 | 指摘分類、修正、再説明 |

---

#### 「状態管理」の分解（ロングタームコンテキスト / 工程の継続性）

**目的:**
開発工程でセッションが途切れても、AI が作業の文脈・未完了・判断理由・次アクションを再構成できるようにする。
看板（ボード）や ToDo チェックリスト、メモリ保存がないと、AI は「どこまでやったか」「何を決めたか」「次に何をするか」を失って破綻する。
したがってここでの状態管理は、アプリの UI 状態ではなく「作業工程の状態（Work State）」を永続化・復元する設計である。
...
| カテゴリ | 内容 |
| --- | --- |
| A. 進捗（Progress） | いまどのフェーズ/ステップにいるか、何が完了で何が未完了か、未完了の理由（ブロッカー/待ち/情報不足） |
| B. タスク構造（Task Graph） | ゴール（達成条件）、エピック/ストーリー/タスク/サブタスクの階層、依存関係（先に必要なこと、並列可能なこと）、優先度と順序（なぜその順なのか） |
| C. 意思決定（Decisions） | 決めたこと（Decision）、代替案と却下理由（Trade-offs）、制約・前提（Constraints / Assumptions）、参照元（リンク、チケット、仕様、ログ） |
| D. 作業中の成果物（Artifacts） | 生成した設計メモ、ADR、ToDo、チェックリスト、コード差分の要点、実行したコマンドと結果、エラーと暫定対応・再現手順 |
| E. 現在地（Session Resume Pointer） | 次にやるべき 1〜3 ステップ（Next actions）、開いていた論点（Open questions）、人間の承認待ち項目（Approval needed）、危険な操作の前提（ロールバック、影響範囲） |

---

**軸 2: 表現形式（人間にも AI にも読みやすい形）**

| 形式 | 説明 |
| --- | --- |
| チェックリスト（ToDo） | 短い動詞 + 完了条件 |
| 看板（Kanban） | Backlog / Ready / Doing / Review / Blocked / Done |
| 逐次ログ（Journal） | いつ何をしたか（時系列） |
| 決定ログ（Decision Log / ADR） | 決めたことと理由 |
| 状態スナップショット（Snapshot） | 再開に必要な最小セット |

実務上は「看板 + 決定ログ + スナップショット」の 3 点セットが強い。
看板は"何をするか"、決定ログは"なぜそうするか"、スナップショットは"今どこか"。

---

**軸 3: 粒度（どの単位で区切って保存するか）**

| 粒度 | 単位 |
| --- | --- |
| 大粒度 | プロジェクト/機能単位（長期） |
| 中粒度 | スプリント/エピック単位（週〜月） |
| 小粒度 | タスク/PR 単位（時間〜日） |
| 最小粒度 | ステップ（数分〜30 分で完了する具体行動） |

> 重要: エージェントの破綻が起きやすいのは「タスクの中の最小ステップ」が保存されていない時。
> 例：「テストを追加する」だけだと再開不能。→「どのファイルに、どの観点の、どの期待値で、どういう失敗を潰すテストを追加するか」まで落とす。
...
| --- | --- |
| ステップ完了時 | チェックをつける |
| 重要な意思決定をした時 | Decision Log に追記 |
| エラーが起きた時 | 再現・原因仮説・対処を記録 |
| PR 作成/更新時 | 差分要点、テスト証跡 |
| セッション終了時 | Snapshot を作る |
| セッション再開時 | Snapshot から復元し、差分を確認 |

---

**軸 5: 復元手順（セッション再開のプロトコル）**

| # | 手順 |
| --- | --- |
| 1 | Snapshot を読む（目的、現在地、次アクション） |
| 2 | 看板で Doing/Blocked を確認 |
| 3 | Open questions と Approval needed を確認 |
| 4 | 直近ログ（Journal）の最後を読む |
| 5 | 実行コマンド/テスト結果を再現できる形で確認 |
| 6 | 次の最小ステップを着手（30 分以内で終わる単位に分割） |

---

**軸 6: 破綻パターンとガードレール（なぜ必要か）**

| 破綻 | 原因 | ガードレール |
| --- | --- | --- |
| 何をやっていたか忘れる | Snapshot 欠如 | Snapshot 必須（Next actions を固定） |
| なぜそうしたか忘れて同じ議論を繰り返す | Decision Log 欠如 | Decision Log 必須（却下理由も残す） |
| 未完了が溜まり、どこから再開すべきか不明 | 看板不在 | 看板で Blocked/Ready を分離、依存を明示 |
| 実行・検証の証跡がなく、再現できない | Artifacts 不足 | コマンド・結果・環境を Artifacts として保存 |
| タスクが大きすぎて進まない | 粒度不適切 | 最小ステップ（30 分単位）に強制分割 |
...
#### （モジュール化の前に必要になりがちな追加の分解）

**A) 「仕様」の分解（= エージェントが迷わず作業できる入力形式にする）**
...
| 例外系 | 失敗、制限、権限、レート、タイムアウト |
| 画面/UX の期待 | 状態遷移、空/読み込み/失敗 |
| 依存 | 外部 API、既存機能、互換性 |
| 計測 | ログ、メトリクス、トレース、KPI |

---

**B) 「変更（PR）」の分解（= 実装の単位ではなく"説明と検証"も含める）**
...
**C) 「運用」の分解（= デプロイ後もエージェントが回せる）**

| 要素 | 説明 |
| --- | --- |
| 監視項目の定義 | 何を見れば成功/失敗か |
| アラートの分類 | 緊急度、担当、初動 |
| ランブック | 切り分け手順、暫定対応、恒久対応 |
| ポストモーテム | 原因、再発防止、アクション |

---

#### 最小作業単位のテンプレ（状態管理モジュール化の前段）

エージェントに委譲可能な最小単位は、以下のフォーマットで定義する。
...
**例:**
```
Step名: APIエンドポイントのテストを追加する
入力: API仕様書 (docs/api-spec.md), 既存テストファイル (tests/api.test.ts)
作業内容: GET /users/:id のレスポンス形式を検証するテストを追加
完了条件: npm test が PASS し、カバレッジが 80% 以上
出力: tests/api.test.ts の差分, テスト実行ログ
失敗時: 型エラー → 型定義を確認, タイムアウト → モックの設定を確認
```

---

#### まとめ（この分解が"オーケストレーション前提"である理由）

大きい概念（テスト/レビュー/計画/状態管理）は、そのままではエージェントが実行できない。

「入力」「処理」「出力」「検証（合否判定）」「失敗時の分岐」を持つ最小作業単位へ分解すると、

- **並列化できる**: 独立したステップは同時実行可能
- **リトライできる**: 失敗したステップだけ再実行
- **人間の承認ポイントを挿入できる**: 危険な操作の前に確認
- **ログで追跡できる**: 何がどこまで進んだか可視化

**したがって、モジュール化は"概念の名前"ではなく、"人間の作業単位"と"合否判定"の境界で行う。**
...
> **SSOT 注記**: コンポーネント一覧の詳細は **BUILD-FROM-SCRATCH.md §6** を参照。本セクションは概要のみ。
...
| --- | --- | --- | --- |
| I-BOOT-1 | 前提欠落 | playbook.issue-log | dependency-checker -> user 補完 |
...
| I-PLAN-FREEZE | plan 凍結違反 | progress.json | plan 変更試行 -> 進行停止 |
