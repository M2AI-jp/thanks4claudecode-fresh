...
- [x] **p1.1**: merge-pr.sh の実装内容が把握されている
  - executor: claudecode
  - validations:
...
- [x] **p1.2**: settings.json に merge-pr.sh が未登録であることが確認されている
  - executor: claudecode
  - validations:
...
- [x] **p1.3**: 「全 milestone achieved」検知ロジックの実装場所が決定されている
  - executor: claudecode
  - validations:
...
- [x] **p2.1**: archive-playbook.sh に project.md 参照ロジックが追加されている
  - executor: claudecode
  - validations:
...
- [x] **p2.2**: 全 milestone が achieved の場合に検知できることが確認されている
  - executor: claudecode
  - validations:
...
- [x] **p2.3**: 検知時に merge-pr.sh 呼び出し or Claude への指示が出力される
  - executor: claudecode
  - validations:
...
- [x] **p3.1**: テスト用 project.md（全 milestone achieved）が作成されている
  - executor: claudecode
  - validations:
...
- [x] **p3.2**: archive-playbook.sh がテスト project.md で正しく動作する
  - executor: claudecode
  - validations:
...
- [x] **p3.3**: 実際のセッションで project_complete が発火することが確認されている
  - executor: claudecode
  - validations:
...
- [x] **p4.1**: repository-map.yaml の project_complete が実態と一致している
  - executor: claudecode
  - validations:
...
- [x] **p4.2**: extension-system.md に project_complete の発火条件が記載されている
  - executor: claudecode
  - validations:
...
- [x] **p_final.1**: archive-playbook.sh に「全 milestone achieved」検知ロジックが追加されている
  - executor: claudecode
  - validations:
...
- [x] **p_final.2**: 全 milestone achieved 時に merge-pr.sh 呼び出し or Claude への指示が出力される
  - executor: claudecode
  - validations:
...
- [x] **p_final.3**: E2E テスト（模擬シナリオ）で project_complete workflow が動作する
  - executor: claudecode
  - validations:
...
- [x] **p_final.4**: repository-map.yaml の project_complete workflow が実態と一致している
  - executor: claudecode
  - validations:
