...
- [x] **p1.1**: archive-playbook.sh が全 Phase done 検出時に自動コミットを実行する
  - executor: claudecode
  - validations:
...
- [x] **p1.2**: archive-playbook.sh が playbook を plan/archive/ に自動移動する
  - executor: claudecode
  - validations:
...
- [x] **p1.3**: archive-playbook.sh が state.md の playbook.active を null に更新する
  - executor: claudecode
  - validations:
...
- [x] **p1.4**: archive-playbook.sh が git push を実行する
  - executor: claudecode
  - validations:
...
- [x] **p1.5**: archive-playbook.sh が create-pr.sh を呼び出す
  - executor: claudecode
  - validations:
...
- [x] **p1.6**: archive-playbook.sh が merge-pr.sh を呼び出す
  - executor: claudecode
  - validations:
...
- [x] **p1.7**: archive-playbook.sh が main ブランチに同期する
  - executor: claudecode
  - validations:
...
- [x] **p1.8**: archive-playbook.sh が post-loop-pending ファイルを作成する（成功時のみ）
  - executor: claudecode
  - validations:
...
- [x] **p1.9**: PR マージ失敗時は pending にステータスを記録する
  - executor: claudecode
  - validations:
...
- [x] **p2.1**: .claude/session-state/ ディレクトリが存在する
  - executor: claudecode
  - validations:
...
- [x] **p2.2**: pre-tool.sh が post-loop-pending を検出し BLOCK する
  - executor: claudecode
  - validations:
...
- [x] **p2.3**: pending 検出のガードスクリプトが存在する
  - executor: claudecode
  - validations:
...
- [x] **p2.4**: pending-guard.sh に許可リスト（allowlist）が存在する
  - executor: claudecode
  - validations:
...
- [x] **p2.5**: pending-guard.sh がステータスに応じた案内を出す
  - executor: claudecode
  - validations:
...
- [x] **p3.1**: post-loop SKILL.md が更新されている
  - executor: claudecode
  - validations:
...
- [x] **p3.2**: post-loop/handlers/complete.sh が pending を削除する
  - executor: claudecode
  - validations:
...
- [x] **p3.3**: post-loop SKILL.md の行動に「最初に complete.sh を実行」と明記されている
  - executor: claudecode
  - validations:
...
- [x] **p3.4**: complete.sh が pending ファイル削除後に state.md を更新する
  - executor: claudecode
  - validations:
...
- [x] **p4.1**: docs/ARCHITECTURE.md に post-loop 自動発火フローが追記されている
  - executor: claudecode
  - validations:
...
- [x] **p4.2**: archive-playbook.sh のコメントが更新されている
  - executor: claudecode
  - validations:
...
- [x] **p_self_update.1**: 実装中の発見事項が playbook に反映されている
  - executor: claudecode
  - validations:
...
- [x] **p_final.1**: archive-playbook.sh が playbook 完了時にアーカイブ/PR 作成/マージを自動実行する
  - executor: claudecode
  - validations:
...
- [x] **p_final.2**: pre-tool.sh が post-loop-pending を検出し、post-loop 実行を強制する
  - executor: claudecode
  - validations:
...
- [x] **p_final.3**: post-loop Skill が次タスク導出のみを担当し、pending を削除する
  - executor: claudecode
  - validations:
